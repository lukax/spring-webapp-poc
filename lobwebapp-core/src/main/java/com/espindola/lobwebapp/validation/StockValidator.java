...
public class StockValidator extends AbstractEntityValidator<Stock> {
	...
	private void validateUnit(Stock t, Errors errors) {
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "unit", MessageKey.STOCKUNITINVALID_VALIDATION.getMessageKey(), new Object[]{3});
		if(t.getUnit() != null && t.getUnit().length() > 3)
			errors.rejectValue("unit", MessageKey.STOCKUNITINVALID_VALIDATION.getMessageKey(),
	...
	private void validateMinQuantity(Stock t, Errors errors) {
		if(t.getMinQuantity() != null && t.getMinQuantity() < 0)
			errors.rejectValue("minQuantity", MessageKey.STOCKMINQUANTITYINVALID_VALIDATION.getMessageKey(),
	...
	private void validateMaxQuantity(Stock t, Errors errors) {
		if(t.getMaxQuantity() != null && t.getMaxQuantity() < 0)
			errors.rejectValue("maxQuantity", MessageKey.STOCKMAXQUANTITYINVALID_VALIDATION.getMessageKey(),
					new Object[] { 0 }, defaultMessage);
	...
	private void validateQuantity(Stock t, Errors errors) {
		if(t.getQuantity() == null || t.getQuantity() < 0)
			errors.rejectValue("quantity", MessageKey.STOCKQUANTITYINVALID_VALIDATION.getMessageKey(),
					new Object[] { 0 }, defaultMessage);
	}
	...
	private void validateProduct(Product product, Errors errors){
		...
		if(product == null || product.getId() == null || product.getId() <= 0){
			errors.rejectValue("product", MessageKey.STOCKPRODUCTINVALID_VALIDATION.getMessageKey(), defaultMessage);
		}
	}
