...
public class UserValidator extends PersonValidator<User> {
	...
	private void validatePassword(User t, Errors e) {
		ValidationUtils.rejectIfEmptyOrWhitespace(e, "password",
				MessageKey.USERPASSWORDINVALID_VALIDATION.getKey());
		if (t.getPassword() != null
				&& (t.getPassword().length() < 8 || t.getPassword().length() > 20))
	...
	private void validateUsername(User t, Errors e) {
		ValidationUtils.rejectIfEmptyOrWhitespace(e, "username",
				MessageKey.USERUSERNAMEINVALID_VALIDATION.getKey());
		if (t.getUsername() != null
				&& (t.getUsername().length() < 6 || t.getUsername().length() > 20))
