      ...
      var ModalFactory = function ModalFactoryFn(config) {
        function Modal(config) {
          ...
          return $q.when($templateCache.get(templateUrl) || $http.get(templateUrl, { cache: true }).then(function (res) {
            ...
            if (options.id == null)
              var id = template.replace('.html', '').replace(/[\/|\.|:]/g, '-') + '-' + scope.$id;
            else
              var id = options.id;
            var $modal = $('<div class="modal" tabindex="-1" role="dialog" aria-hidden="true"></div>').attr('id', id).attr('aria-labelledby', id).html(template);
