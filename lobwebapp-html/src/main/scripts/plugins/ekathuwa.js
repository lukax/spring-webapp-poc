 ...
 */
'use strict';
define(['angular', 'angularRoute', 'jquery', 'bootstrap'], function (angular, angularRoute, $, bootstrap) {
        ...
        .provider('$ekathuwa', function () {
            this.$get = function ($compile, $rootScope, $timeout, $q) {
                this.modal = function (op) {
                    ...
                    $("#ekathuwaSt" + op.id).remove();
                    $('head').append('<style id="ekathuwaSt' + op.id + '">' + mq + '</style>');
                    $("#" + op.id).remove();
                    var m = $(t);
                    $('body').append(m);
                    $compile(m)(op.scope);
