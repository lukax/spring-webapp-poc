...
module entity {
  export class EntityServiceMock<T extends AbstractEntity> implements EntityService<T> {
    ...
    list(successCallback:(data:T[], status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any, errorCallback:(data:MessageResponse, status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any, pageable?:Page) {
      this.$timeout(() => {
