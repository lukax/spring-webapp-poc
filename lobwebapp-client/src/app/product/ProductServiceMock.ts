...
module product {
  export class ProductServiceMock extends core.EntityServiceMock<product.Product> implements ProductService {
    ...
    findByName(name:string, successCallback:(data:Product[], status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any, errorCallback:(data:core.MessageResponse, status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any, pageable:core.Page) {
      this.$timeout(()=> {
    ...
    listCategory(successCallback:(data:string[], status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any, errorCallback:(data:core.MessageResponse, status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any) {
      this.$timeout(() => {
        var categories = [];
...
//product.module.service("ProductService", product.ProductServiceMock);
