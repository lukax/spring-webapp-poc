...
module order.graph {
  export interface IGraphOrderController extends core.IController {
  }

  interface XOrder extends order.Order {
    dd: Date;
    month: Date;
    total: number;
    productQt: number;
  ...
  export class GraphOrderController implements IGraphOrderController {
    ...
    buildGraph(orders:order.Order[]) {
      ...
      orders.forEach((o:XOrder) => {
        ...
        o.total = 0;
        o.productQt = 0;
        o.items.forEach((item:order.OrderItem) => {
      ...
      var paymentStatusDimension = data.dimension((d:order.Order)=> {
...
order.graph.module.controller("GraphOrderController", order.graph.GraphOrderController);
