...
module service.impl {
  export class ProductServiceImpl extends base.EntityServiceImpl<domain.Product> implements service.contract.ProductService, service.contract.base.HasDefaultValue<domain.Product> {
    ...
    findByName(name:string, successCallback:(data:domain.Product[], status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any, errorCallback:(data:domain.util.MessageResponse, status:number, headers:(headerName:string) => string, config:ng.IRequestConfig) => any, page:domain.util.Page) {
      var params = {
        name: "%" + name + "%"
      };
