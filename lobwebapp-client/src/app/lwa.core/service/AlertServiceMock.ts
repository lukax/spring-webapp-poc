...
class AlertServiceMock implements service.contract.AlertService {
    ...
    add(alert: domain.util.Alert) {
        ...
        if (!alert.type) alert.type = enumUtil.AlertType.OK;
    ...
    addMessageResponse(messageResponse: domain.util.MessageResponse, title: string) {
        ...
        var alert: domain.util.Alert = { content: messageResponse.message, title: title, type: enumUtil.AlertType.DANGER, date: new Date() }
        ...
        this.$interval(() => {
            ...
            alerts.forEach((x, index) => {
                ...
                if (differenceInSecs > this.lifeSecs) {
                    if(x.type != enumUtil.AlertType.DANGER)
                        alerts.splice(index, 1);
                }
                });
            this.setAlerts(alerts);
