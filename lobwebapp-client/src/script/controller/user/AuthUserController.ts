...
export class AuthUserController implements IAuthUserController {
    ...
    login() {
        ...
        this.AuthService.login(this.user,
            ...
            () => {
                this.AlertService.add({ title: "Login", content: "Usuário ou senha inválido", type: enumUtil.AlertType.WARNING });
                this.lock = false;
            });
    }
    ...
    logout() {
        ...
        this.AuthService.logout(
            ...
            (errorData) => {
                ...
                this.AlertService.add({ title: "Logout", content: String(errorData.message), type: enumUtil.AlertType.WARNING });
                this.lock = false;
            });
    }
    ...
    private processParams() {
        ...
        switch (String(error)) {
            case "0":
                this.AlertService.add({ content: "Login ou senha Inválido", type: enumUtil.AlertType.WARNING });
                this.AuthService.logout(()=>{}, ()=>{});
                break;
            case "1":
                this.AlertService.add({ content: "Você não possui permissão para acessar esta página", type: enumUtil.AlertType.WARNING });
                this.AuthService.logout(()=>{}, ()=>{});
                break;
        }
